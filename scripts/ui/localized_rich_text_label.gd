# D:\Works\Godot\ADVEngineDemo\scripts\ui\localized_rich_text_label.gd
@tool
extends RichTextLabel
class_name LocalizedRichTextLabel

## 自動的に翻訳されるRichTextLabel
## インスペクターで translation_key を設定するだけで使用可能
##
## 使用方法:
## 1. RichTextLabelの代わりにLocalizedRichTextLabelノードを作成
## 2. インスペクターで Translation Key に "Messages.welcome" などを設定
